  ...
  modules: [
    {
      slug: "sources",
      name: "数据采集",
      summary: "把知识文档与业务数据汇入数据中台，沉淀为可检索、可查询的资产。",
      status: "building",
      notice:
        "「知识文档 → RAG 知识库」这条链路已支持 md / txt / docx / pdf。扫描件 PDF（没有文字层）需要 OCR，暂不支持；业务数据文件的导入、采集调度与失败重试尚未建立。",
      current: [
        "知识文档上传已接入：支持 md / txt / docx / pdf，切片、向量化后写入知识库",
        "docx 按段落样式、PDF 按字号还原出标题层级，再复用同一套切片逻辑",
        "入库幂等：内容未变时整体跳过，不重复消耗向量计算",
        "已入库文档列表可查，含切片数与更新时间",
      ],
      upcoming: [
        "扫描件 PDF 的 OCR（目前只支持带文字层的 PDF）",
        "业务数据文件（Excel / CSV）导入为数据仓库表",
        "采集任务调度、运行状态与失败重试",
      ],
      dependencies: [
        "业务中台（客户 / 商品 / 订单 / 库存）→ 数据采集 → ODS 层",
        "知识文档（口径 / 规范 / 字典）→ 数据采集 → RAG 知识库",
      ],
    },
    {
      slug: "governance",
